...
# Define event tokens (form-based, no functional distinction)

EVENT_TOKENS = {
    "eh", "ehm", "ah", "oh",
    "mh", "mhmh", "mm",
    "okay", "sì", "va", "bene", "esatto",
    "cioè", "diciamo"
}
...
def compute_score(trascrizione_gold_path, trascrizione_whi_path, out_alignment_dir=None, file_id=None):
    ...
    if out_alignment_dir is not None and file_id is not None:
        os.makedirs(out_alignment_dir, exist_ok=True)
        out_path = os.path.join(out_alignment_dir, f"{file_id}.align.tsv")
    ...
    for (_, g_w), (_, w_w) in zip(aligned_gold, aligned_whisper):

        is_event = (g_w in EVENT_TOKENS) or (w_w in EVENT_TOKENS) # guarda se il token è un evento
        if not is_event:
            continue

        event_total += 1 # se lo è, top lo considera
...
def evaluate_dataset_with_pruning(trial, audio_paths, references, params, args, chunk_size=7):
    ...
    for i in range(0, len(audio_paths), chunk_size):
        ...
        for ap, ref in zip(chunk_paths, chunk_refs):
            # SET ARGS:
            args.project_dir = f"eventi_configuration_results/A/{ap}_from20_A" #! CHANGE HERE IF CHANGES ARE MADE FOR STEPS 1-3
            args.input = f"audio_from20/subset_A/{ap}.wav"
            ...
            # print("ARGS:", args)
            # input()
            alignment_dir = f"{args.project_dir}/alignments/trial_{trial.number}"
            
            res = compute_score(
                ref,
                f"{args.project_dir}/subs_final_{trial.number}/podcast_clean_validated.txt",
                out_alignment_dir=alignment_dir,
                file_id=ap
            )
            
            if res["event_total"] > 0:
                suppression = (
                    res["event_del"] + res["event_sub"]
                ) / res["event_total"] # quanto spesso vengono persi
                production = res["event_ok"] / res["event_total"] # quanto spesso vengono prodotti correttamente
            else:
                suppression = 0.0
                production = 0.0
            ...
            loss = (
                res["wer"]
                + lambda_event * suppression # penalizza se cancella o normalizza eventi
                - mu_event * production # premia se li produce correttamente
            )

            losses.append(loss)

        interim = float(np.mean(losses))
    ...
    return float(np.mean(losses))
            ...
            #wers.append(compute_score(ref,
                             #f"{args.project_dir}/subs_final_{trial.number}/podcast_clean_validated.txt", out_alignment_dir=alignment_dir,file_id=ap))
...
def main(args):
    ...
    val_audio = ["BOC1002","KPS023","KPS024","PBB010","PBB024","PBC017","PTA016","PTA018", "PTD003","TOA1005","TOA3010","TOC1001"]
    # val_audio = ["BOC1002", "BOC1003", "BOC1005"]
    # BOC1002/subs_final/podcast_clean_validated.txt
    val_text = [f"GOLD/from20min/subset_A/{conv_id}.txt" for conv_id in val_audio]
